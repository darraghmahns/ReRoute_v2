...
def get_route(
    ...
    route = (
        db.query(Route)
        .filter(Route.id == route_id, Route.user_id == current_user.id)
        .first()
    )
...
def save_route(
    ...
    saved_route = SavedRoute(
        user_id=current_user.id, route_id=save_data.route_id, notes=save_data.notes
    )
...
def get_saved_routes(
    ...
    saved_routes = (
        db.query(SavedRoute).filter(SavedRoute.user_id == current_user.id).all()
    )
