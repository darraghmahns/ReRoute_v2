    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if token_info.get("expires_at"):
                profile.strava_token_expires_at = datetime.fromtimestamp(token_info["expires_at"])
            db.commit()
...
def get_auth_url():
    ...
    logging.info(f"Strava config - Client ID: {settings.STRAVA_CLIENT_ID}, Redirect URI: {settings.STRAVA_REDIRECT_URI}")
...
async def handle_callback(request: Request, current_user: User = Depends(get_current_active_user_by_session), db: Session = Depends(get_db)):
    ...
    try:
        ...
        logging.info(f"Attempting token exchange with client_id: {settings.STRAVA_CLIENT_ID}")
        response = requests.post(token_url, data=token_data)
        
        if response.status_code != 200:
            error_detail = response.text
            logging.error(f"Failed to exchange code for token. Status: {response.status_code}, Response: {error_detail}")
...
@router.post("/sync")
def sync_activities(current_user: User = Depends(get_current_active_user_by_session), db: Session = Depends(get_db)):
    ...
    try:
        ...
        access_token = get_valid_strava_token(profile, db)
        if not access_token:
            raise HTTPException(status_code=400, detail="Strava not connected or token invalid")
        ...
        while len(all_activities) < 200 and page <= 4:
            ...
            if response.status_code != 200:
                logging.error(f"Failed to fetch Strava activities page {page}: {response.status_code} - {response.text}")
                break
            ...
            logging.info(f"Fetched {len(activities)} activities from page {page} (total: {len(all_activities)})")
            page += 1
        ...
        if all_activities:
            ...
            activity_dates = [act.get('start_date', 'Unknown')[:10] for act in all_activities[:5]]
            ...
            for act in all_activities:
                ...
                if streams_resp.status_code == 200:
                    ...
                    logging.info(f"Fetched streams for {act_name}: {', '.join(available_streams)}")
                    ...
                    logging.warning(f"Failed to fetch streams for {act_name}: {streams_resp.status_code}")
            ...
            logging.info(f"Sync complete: {new_activities} new, {updated} updated, {upserted} total")
...
def refresh_all_activities(current_user: User = Depends(get_current_active_user_by_session), db: Session = Depends(get_db)):
    ...
    try:
        ...
        deleted_count = db.query(StravaActivity).filter(StravaActivity.user_id == current_user.id).delete()
        logging.info(f"Cleared {deleted_count} existing activities for user {current_user.id}")
        ...
        if all_activities:
            ...
            db.rollback()
            raise HTTPException(status_code=400, detail="Failed to fetch activities for refresh")
        ...
        raise HTTPException(status_code=500, detail=f"Error refreshing activities: {str(e)}")

@router.get("/activities")
def get_activities(current_user: User = Depends(get_current_active_user_by_session), db: Session = Depends(get_db)):
...
@router.post("/zones")
def fetch_zones(current_user: User = Depends(get_current_active_user_by_session), db: Session = Depends(get_db)):
    """Fetch athlete zones"""
    try:
        profile = db.query(Profile).filter(Profile.id == current_user.id).first()
...
def disconnect(current_user: User = Depends(get_current_active_user_by_session), db: Session = Depends(get_db)):
    """Disconnect Strava"""
    try:
        profile = db.query(Profile).filter(Profile.id == current_user.id).first()
...
def get_activities_db(current_user: User = Depends(get_current_active_user_by_session), db: Session = Depends(get_db)):
...
def get_activity_streams(activity_id: int, current_user: User = Depends(get_current_active_user_by_session), db: Session = Depends(get_db)):
