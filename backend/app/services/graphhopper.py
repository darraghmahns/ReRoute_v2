...
class GraphHopperService:
    ...
    def generate_route(self, params: RouteGenerationParams) -> Dict[str, Any]:
        ...
        if custom_model:
            logger.info(
                f"Custom model priority rules: {len(custom_model.get('priority', []))} rules for route_type={params.route_type}"
            )
    ...
    def _build_custom_model(
        ...
        if params.route_type == "road" or params.profile == "bike":
            logger.info(
                "Using ultra-strict custom model to force paved roads only for road bikes"
            )
            return {
                "speed": [
                ...
                "priority": [
                    ...
                    {
                        "if": "road_class == PRIMARY && (surface == ASPHALT || surface == CONCRETE || surface == PAVED)",
                        "multiply_by": "2.0",
                    },
                    {
                        "if": "road_class == SECONDARY && (surface == ASPHALT || surface == CONCRETE || surface == PAVED)",
                        "multiply_by": "1.8",
                    },
                    {
                        "if": "road_class == TERTIARY && (surface == ASPHALT || surface == CONCRETE || surface == PAVED)",
                        "multiply_by": "1.5",
                    },
                    {
                        "if": "road_class == RESIDENTIAL && (surface == ASPHALT || surface == CONCRETE || surface == PAVED)",
                        "multiply_by": "1.3",
                    },
    ...
    def _snap_to_road(
        self, lat: float, lng: float, profile: str = "bike"
    ) -> Tuple[float, float]:
    ...
    def generate_ai_loop_route(
        ...
        try:
            ...
            logger.info(
                f"Targeting {distance_km:.1f}km loop from {start_lat:.4f},{start_lng:.4f}"
            )
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    base_waypoint_distance_km = (
                        distance_km / 4
                    )  # Start with 1/4 of target distance
                    waypoint_distance_km = base_waypoint_distance_km * (
                        1 + attempt * 0.5
                    ...
                    waypoint_bearing = 90 + (
                        attempt * 45
                    )  # Vary bearing with each attempt
                    ...
                    try:
                        waypoint_lat, waypoint_lng = self._snap_to_road(
                            waypoint_lat, waypoint_lng, profile
                        )
                    except Exception as e:
                        logger.warning(
                            f"Road snapping failed for waypoint, using original coordinates: {e}"
                        )
                        # Continue with original coordinates

                    logger.info(
                        f"Attempt {attempt + 1}: Waypoint at {waypoint_distance_km:.1f}km, bearing {waypoint_bearing}°"
                    )
                    ...
                    loop_params = RouteGenerationParams(
                        ...
                        waypoints=[
                            RouteWaypointCreate(
                                lat=waypoint_lat, lng=waypoint_lng, waypoint_type="via"
                            )
                        ],
                    )
                    ...
                    if "paths" in response and response["paths"]:
                        ...
                        logger.info(
                            f"Attempt {attempt + 1}: Generated {actual_distance_km:.1f}km route, target {distance_km:.1f}km, diff={distance_ratio:.1%}"
                        )
