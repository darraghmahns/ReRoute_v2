...
class StravaRouteIntegration:
    ...
    def get_user_popular_segments(self, user_id: str, db: Session, 
                                 lat: float, lng: float, radius_km: float = 50) -> List[Dict[str, Any]]:
    ...
    def get_user_activity_heatmap_data(self, user_id: str, db: Session) -> Dict[str, Any]:
        ...
        sorted_areas = sorted(coordinate_frequency.items(), key=lambda x: x[1], reverse=True)
    ...
    def enhance_route_with_strava_data(self, route_geometry: Dict[str, Any], 
        ...
        popularity_score = total_popularity / len(sampled_coords) if sampled_coords else 0.0
    ...
    def suggest_route_modifications(self, route_geometry: Dict[str, Any], 
                                  user_id: str, db: Session) -> List[Dict[str, Any]]:
    ...
    def _process_segments(self, segments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for segment in segments:
            processed.append({
                ...
                "popularity_score": min(segment.get("effort_count", 0) / 1000, 1.0)  # Normalize
            })
    ...
    def _segment_intersects_route(self, segment: Dict[str, Any], 
                                route_coords: List[List[float]]) -> bool:
