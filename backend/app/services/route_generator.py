...
class RouteGenerationService:
    ...
    def generate_route(self, params: RouteGenerationParams, user_id: str, db: Session) -> Dict[str, Any]:
    ...
    def generate_ai_loop_route(self, start_lat: float, start_lng: float, 
                              distance_km: float, profile: str, route_type: str,
                              num_waypoints: int, user_id: str, db: Session) -> Dict[str, Any]:
    ...
    def _process_route_response(self, gh_response: Dict[str, Any], 
                               params: RouteGenerationParams) -> Dict[str, Any]:
        ...
        if "points" in path:
            logger.info(f"Points structure: {list(path['points'].keys()) if isinstance(path['points'], dict) else 'not dict'}")
            if "coordinates" in path["points"] and len(path["points"]["coordinates"]) > 0:
                coord_sample = path["points"]["coordinates"][0]
                logger.info(f"Sample coordinate: {coord_sample} (length: {len(coord_sample)})")
    ...
    def _calculate_elevation(self, path: Dict[str, Any]) -> tuple[float, float]:
        ...
        if not elevations and "details" in path:
            if "elevation" in path["details"]:
                ...
                elevations = [detail[2] if len(detail) >= 3 else 0 for detail in elevation_details]
        ...
        logger.info(f"Calculated elevation: gain={elevation_gain:.1f}m, loss={elevation_loss:.1f}m")
        return elevation_gain, elevation_loss
    
    def _create_route_record(self, route_data: Dict[str, Any], 
                            params: RouteGenerationParams, user_id: str,
                            gh_response: Dict[str, Any]) -> Route:
    ...
    def _calculate_difficulty(self, route: Route, params: RouteGenerationParams) -> float:
        """Calculate route difficulty score"""
        return min(10.0, (
            (route.distance_m / 1000) * 0.5 +  # Distance factor
            (route.total_elevation_gain_m / 100) * 1.0 +  # Elevation factor
            (2.0 if params.route_type == "mountain" else 1.0)  # Terrain factor
        ))
