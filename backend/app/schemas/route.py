...
class RouteGenerationParams(BaseModel):
    ...
    end_lat: Optional[float] = Field(
        None, description="Ending latitude (optional for loops)"
    )
    end_lng: Optional[float] = Field(
        None, description="Ending longitude (optional for loops)"
    )
    profile: str = Field(..., description="Routing profile: bike, gravel, mountain")
    route_type: str = Field(
        ..., description="Route type: road, gravel, mountain, urban"
    )
    distance_km: Optional[float] = Field(
        None, description="Target distance in kilometers"
    )
    is_loop: bool = Field(False, description="Generate a loop route")
    avoid_highways: bool = Field(True, description="Avoid highways")
    elevation_preference: Optional[str] = Field(
        None, description="flat, rolling, hilly"
    )
    waypoints: Optional[List[RouteWaypointCreate]] = Field(
        [], description="Intermediate waypoints"
    )
    use_strava_segments: bool = Field(
        False, description="Prefer popular Strava segments"
    )
...
class RouteResponse(RouteBase):
    ...
    waypoints: Optional[List[Dict[str, Any]]] = Field(
        None, description="Route waypoints"
    )
    elevation_profile: Optional[List[Dict[str, Any]]] = Field(
        None, description="Elevation data"
    )
    distance_m: float
    ...
    end_lng: float
    is_loop: bool
    strava_segments: Optional[List[Dict[str, Any]]] = Field(
        None, description="Strava segments"
    )
    popularity_score: Optional[float] = Field(
        None, description="Route popularity score"
    )
    created_at: datetime
    updated_at: datetime
