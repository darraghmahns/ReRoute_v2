  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      if (error.response?.status === 401) {
        ...
        throw new Error(
          'Please check your email format and ensure password is at least 6 characters.'
        );
      ...
      if (error.response?.status === 400) {
        ...
        if (typeof detail === 'string') {
          if (detail.includes('email') && detail.includes('already')) {
            ...
            throw new Error(
              'Password does not meet requirements. Please ensure it has at least 6 characters with uppercase, lowercase, and number.'
            );
          } else {
        ...
        throw new Error(
          'Please check your email format and ensure password meets requirements.'
        );
      } else if (error.response?.status === 500) {
        throw new Error('Server error. Please try again later.');
      } else if (
