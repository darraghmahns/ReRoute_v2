...
import { User as UserIcon, Calendar, MapPin, Clock, TrendingUp, Edit, Camera, Activity } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle } from '../components/ui/Card';
import { Button } from '../components/ui/Button';
import { getProfile, getStravaActivities, calculateStats, getRecentActivities, formatDistance, formatTime, formatElevation } from '../services/profile';
                ...
                <div className="flex items-center space-x-6">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-4 mb-2">
                      ...
                      <Button variant="outline" size="sm" className="border-reroute-gray text-white hover:bg-reroute-card" onClick={() => setEditOpen(true)}>
                        <Edit className="w-4 h-4 mr-2" />
                        Edit Profile
                      </Button>
                    </div>
                    <p className="text-gray-400 mb-2">
                      {profile?.cycling_experience ? `${profile.cycling_experience} cyclist` : 'Cycling enthusiast'}
                      {profile?.fitness_level && ` • ${profile.fitness_level} fitness level`}
                    </p>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                         ...
                         recentActivities.map((activity) => (
                          <div key={activity.id} className="flex items-center space-x-3 p-3 bg-reroute-primary/10 rounded-lg">
                        <div className="w-2 h-2 bg-reroute-primary rounded-full"></div>
                            <div>
                              ...
                              <p className="text-xs text-gray-400">
                                {new Date(activity.start_date).toLocaleDateString()} - {formatDistance(activity.distance_m * 0.000621371)}
                              </p>
                            </div>
                          </div>
                        ))
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-sm text-gray-500 mt-2">Connect your Strava account to see your activities</p>
                        </div>
                      )}
                    </div>
                  </CardContent>
                      ...
                      <div className="space-y-4">
                        <div>
                          <div className="flex justify-between text-sm mb-2">
                            ...
                            <span className="text-white">{formatDistance(stats.totalDistance)}</span>
                          </div>
                          <div className="w-full bg-reroute-card rounded-full h-2">
                            <div className="bg-reroute-primary h-2 rounded-full" style={{ width: '100%' }}></div>
                          </div>
                        </div>
                        <div>
                          <div className="flex justify-between text-sm mb-2">
                          ...
                          <div className="w-full bg-reroute-card rounded-full h-2">
                            <div className="bg-reroute-green h-2 rounded-full" style={{ width: '100%' }}></div>
                          </div>
                        </div>
                        <div>
                          <div className="flex justify-between text-sm mb-2">
                          ...
                          <div className="w-full bg-reroute-card rounded-full h-2">
                            <div className="bg-reroute-purple h-2 rounded-full" style={{ width: '100%' }}></div>
                          </div>
                        </div>
                      </div>
                    </CardContent>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between p-3 bg-reroute-primary/10 rounded-lg">
                          ...
                          <div className="text-right">
                            <p className="text-lg font-bold text-white">{stats.averageSpeed.toFixed(1)} mph</p>
                            <p className="text-xs text-reroute-green">+2.3 mph</p>
                          </div>
                        </div>
                        <div className="flex items-center justify-between p-3 bg-reroute-green/10 rounded-lg">
                          <div>
                            ...
                            <p className="text-xs text-gray-400">Single activity</p>
                          </div>
                          <div className="text-right">
                            <p className="text-lg font-bold text-white">{formatDistance(stats.maxDistance)}</p>
                        ...
                        <div className="flex items-center justify-between p-3 bg-reroute-purple/10 rounded-lg">
                          ...
                          <div className="text-right">
                            <p className="text-lg font-bold text-white">{formatElevation(stats.totalElevation)}</p>
